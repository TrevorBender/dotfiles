...
-- move mouse to middle top
local safeCoords = {x=960, y=0} -- middle top (1920x1200)
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ modkey }, "p", 
        function () --awful.util.spawn ("dmenu_run") 
            awful.util.spawn("dmenu_run -i -p 'Run command:' -nb '" .. beautiful.bg_normal ..
                "' -nf '" .. beautiful.fg_normal .. "' -sb '" .. beautiful.bg_focus ..
...
clientkeys = awful.util.table.join(
    ...
    awful.key({ modkey }, ",",
        function (c)
            local screen_tags = screen[mouse.screen]:tags()
            local curidx = awful.tag.getidx(c:tags()[1])
            if curidx == 1 then
                c:tags({screen_tags[table.getn(screen_tags)]})
            else
                c:tags({screen_tags[curidx - 1]})
            end
        end
    ),
    awful.key({ modkey }, ".",
        function (c)
            local screen_tags = screen[mouse.screen]:tags()
            local curidx = awful.tag.getidx(c:tags()[1])
            if curidx == table.getn(screen_tags) then
                c:tags({screen_tags[1]})
            else
                c:tags({screen_tags[curidx + 1]})
            end
        end
